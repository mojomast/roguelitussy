...
- `HUD` keeps HP and energy text authoritative and immediate, while the visible bar fills interpolate toward target values in `_Process(...)` and apply a short deterministic pulse tint on damage, healing, and energy changes. Tests assert the target/displayed state directly rather than depending on rendered frames.
