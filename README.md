...
The current build includes character identity and progression, archetype-gated starts, typed seed and daily-challenge entry from the title flow, relic and meta-progression UI, ascension controls, content-authored synergies, deterministic daily seed tracking, narrative run epitaphs, boss phase transitions with UI feedback, faction reputation state, ability casting, gear-driven combat with clearer critical-hit feedback, role-specific AI, equipment requirements, floor-clear rewards, authoritative GameManager-owned visibility, refreshed CC0 0x72 dungeon art with contextual wall layering, committed SVG item/status icons, clearer menu and inventory presentation, text-fitting safeguards for modal UI, prominent HUD/action/combat-log feedback, reconciled world-space movement, overlay-aware gameplay chrome, expanded enemy sprite coverage, and broader mid- and late-floor content.

The July 2026 stabilization checkpoint advances saves to version 17, persists behavior-critical relic hook state, cumulative applied relic-stat totals, and scheduler order zero, hardens meta/daily persistence, routes more damage sources through relic hooks, improves deterministic generation and AI behavior, and adds attack/death/heal/pickup feedback. Status ticks produce standard combat/death events even on skipped turns, all player death sources preserve the player entity for run-finalization invariants, and kill/floor relic milestones no longer trigger late or stack on reentry. Remaining follow-ups, including functional special-room population, boss-template selection, remaining authored relic semantics, and authored enemy sprite-path rendering, are tracked in `docs/IMPROVEMENT_SUGGESTIONS.md`.
