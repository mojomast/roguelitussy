...
- Completed: heal-on-kill and flat damage-bonus synergy effects are live. `echo_bonus` remains unsupported.
- Completed: landmark metadata/fallbacks are reliable, and generation validates lock/key solvability. Requested landmark depth semantics remain a genuine follow-up where authored constraints exceed the fallback contract.
- Completed: player ranged weapon input uses authored range and line-of-sight checks. The final run contract is nine floors, with boss/act feedback, canonical `Victory` completion, and idempotent first-clear unlock. Ascension currently applies only the effects wired in runtime; the ten-level catalog is not ten implemented modifiers.
- Completed: daily challenges are retryable with persisted attempts and best scores. Thursday's authored speed-score modifier is live; other daily modifiers remain unsupported/upcoming.
- Completed: Friendly Merchants' Guild reputation discounts purchases, Orin's dialogue is corrected, `WaitAction` recovers 1 HP unless dangerous statuses suppress recovery, and first-delve/stairs/death guidance plus `Retry Seed`/`New Build` actions are available.
- Verification: full strict suite 734/734; rendering validation 663/663; real Godot 4.5.2 API build with zero warnings/errors; format and diff checks passed. No additional tests were needed for this documentation-only update.

### Archetype, Heritage, And Item Art Follow-up - 2026-09-08

- Completed: corrected new-run archetype drift. `ArchetypeDefinitions` now supplies class base stats and starter package exactly once; menu preview uses the same source plus origin/trait/training modifiers. Existing saves are deliberately not stripped of historical duplicated inventory/stat state.
- Completed: added immutable simulation-side race definitions using existing ability content. Human/Elf/Dwarf/Orc grant War Cry/Phase Shift/Ground Slam/Heavy Slam respectively. Class slots remain first, heritage is appended only when missing, and post-load reconciliation preserves cooldowns, existing slots, saves, and RNG.
- Completed: added an owned-technique palette (`B`) that submits existing `CastAbilityAction`s or enters the existing targeting overlay. It exposes stable deduplicated player slots with target/range/energy/cooldown information, never binds casting to movement, and does not expose generic unowned abilities.
- Completed: authored item paths are now projected to runtime templates, cached through `ItemVisualCatalog`, and displayed in inventory and visible ground piles. Player world bodies render at source 0x72 palette rather than full-body race tint; Ranger/Arcanist use their intended live portrait mappings. No new external assets were imported.
- Capture verification: real Godot 4.5.2/Xvfb output under `/tmp/opencode/roguelitussy-identity-1280` and `/tmp/opencode/roguelitussy-identity-640` shows ability palette, authored inventory icons, and a stack-marked ground pile with no label containment failures. The fixture still warns of CanvasItem/ObjectDB leaks at shutdown; no root cause is yet identified.
- Verification: focused ability/race/item visual tests and real Godot API compile pass. Full strict tests pass 701/701; rendering validation passes 631/631. Formatting and diff checks remain required before merge.
- Remaining identity work: equipment-on-character art, broader legally sourced portrait coverage, Core ability-ownership semantics compatible with scrolls/AI, explicit ranged-weapon command, true Trickster streak reset/reward cadence, and MP terminology/rules.

### Run Pacing And Reward Wiring - 2026-09-08

- Completed: shrine event IDs now survive generation into transient level metadata and create exact authored shrine entities. Cursed-chest spawn points use `curse_room_chest_loot`; safe floors have one deterministic potion and sanctuary cache on distinct nonstair tiles.
- Completed: shrine stat/perk/relic rewards are no longer stranded in `ShrineComponent`. Stat/perk rewards grant existing progression resources; relic offers are deterministic from world/shrine state, reject arbitrary IDs, clear only their source shrine after a valid claim, and re-emit after load while pending. This uses existing shrine persistence, so save version remains 18.
- Completed: late ordinary pools retain five existing credible roles to avoid a floor-11 four-template plateau. Boss pool/tag rules remain unchanged. Orin now provides 22g/30HP emergency care in addition to trading.
- Verification: focused strict shrine/landmark tests pass; the full strict suite passes 685/685 and rendering validation passes 616/616. The increased count includes the landmark/reward and late-pacing regressions.
- Remaining balance risks: rest has no base healing despite its presentation, safe floors do not guarantee that a requested special prefab fits, key/door solvability remains partial, late loot tables are still generic after depth four, much authored synergy behavior remains no-op, and perk choice remains an uncurated full list. These are next-pass targets, not excuses to inflate starting stats.

### NPC, Input, Floor Identity, And UI Follow-up - 2026-09-08

- Completed: added content-validated conditional dialogue (`injured`, `missing_item`, and faction-reputation gates), explicit valid outcomes, per-NPC cosmetic greeting rotation, and bounded branch navigation. Sen now provides a read-only expedition report using current player facts without revealing map/enemy state or consuming RNG.
- Completed: added `NpcServiceAction` for authored paid field dressing. It validates the living registered player, cardinal adjacency to a living neutral provider, service authorization, wounds, and funds both before and during execution. It charges gold and heals via a normal turn, so enemy responses and standard presentation notifications remain consistent. Save version remains 18 because all effects use existing persisted state.
- Completed: expanded the depth-gated roster from Vale/Sen to include Sister Ilex (crypt field medic, depths 3-6) and Cinder Broker Orin (deep outfitter, depths 7+). Current generation picks up new NPCs on newly created floors; cached floors deliberately retain their original population.
- Completed: addressed the player-control asymmetry where AI/simulation moved diagonally while arrows were cardinal-only. `V` followed by two perpendicular arrows/WASD directions selects one diagonal movement/melee action; navigation/numpad diagonals are direct alternatives and numpad 5 waits. Existing corner restrictions, enemy AI, range roles, and cardinal run automation are unchanged.
- Completed: made prison, crypt, and magma floors visibly distinct through terrain-only depth palettes, matching wall-cover/trim colors, deterministic depth-salted wear, and locked-door art/lock cues. Existing 0x72 cell alignment, source scale, contextual wall selection, crop regions, and Z layering are unchanged.
- Completed: fixed observed title-preview, inventory header/detail, dialog/shop, status-icon, and compact-screen overlaps. Single-line labels use real Godot clipping/ellipsis, prose has bounded scroll regions, menu selection is rendered from the actual card height, panel trim uses `SelfModulate`, and compact gameplay collapses duplicate upper HUD stats so log, feedback, HP/XP, and hotbar have separate regions.
- Completed: added `Scenes/Tests/VisualCapture.tscn` plus `Scripts/Tools/VisualCapture.cs`, excluded from stub compilation. It uses a real Godot renderer to capture title, inventory, shop, dialog, Sen report, depth-themed floors, and NPC conversation states. At 640x360 and 1280x720, `--assert-layout` passed with no visible label outside its direct parent; captures live under `/tmp/opencode/roguelitussy-final-640` and `/tmp/opencode/roguelitussy-final-1280` for this workspace run. The fixture enters dialogs through normal `F` routing after isolated test positioning.
- Research: the active terrain is a selected older subset of Robert Norenberg/0x72's CC0 DungeonTileset II. Its current upstream v1.7 autotile atlas is not a drop-in replacement and reintroduces vertical-wall alignment constraints. The official CC0 Sewers expansion is the leading coherent next biome candidate but costs $2 and was not acquired or added. Exact sources, safe integration rules, Godot layout practices, and repeatable capture commands are documented in `docs/VISUAL_VALIDATION.md`.
- Verification: full strict stub suite passes 672/672; rendering validation passes 603/603; real Godot 4.5.2 API build passes with zero warnings/errors; real OpenGL/Xvfb captures at 640x360, 1280x720, and 1920x1080 were inspected. `git diff --check` passed before this documentation update. Godot exits report CanvasItem/ObjectDB leak warnings from the fixture/application shutdown; the source is not yet isolated.
- Next requested pass: investigate stagnation after early floors, pacing/economy/encounter balance, sustainable roguelite mechanics, and distinct classes/races. Start with telemetry/content/replay research before adding mechanics; avoid invalidating the current save/determinism contracts.
